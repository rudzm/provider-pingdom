...
import (
	...
	"github.com/mrudz/provider-pingdom/apis"
	"github.com/mrudz/provider-pingdom/apis/v1alpha1"
	"github.com/mrudz/provider-pingdom/config"
	"github.com/mrudz/provider-pingdom/internal/clients"
	"github.com/mrudz/provider-pingdom/internal/controller"
	"github.com/mrudz/provider-pingdom/internal/features"
)

func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Pingdom").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-pingdom"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-pingdom",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add Pingdom APIs to scheme")
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup Pingdom controllers")
